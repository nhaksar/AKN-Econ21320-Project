...
main_df$alc_and_drugs_yrs_w_both_only <- main_df$A + main_df$D
...
######## Running stuff on just years w/ both alcohol & drugs ###########
##### OLS REGRESSIONS ON TOTAL LAYOFFS #####
mod_nofe_nolag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff, data=panel_df)
mod_nofe_1lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1),
                     data=panel_df)
mod_nofe_2lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2), data=panel_df)
mod_nofe_3lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3), data=panel_df)
mod_nofe_4lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + 
                       lag(Total_layoff, 1) + lag(Total_layoff, 2) + 
                       lag(Total_layoff, 3) + lag(Total_layoff, 4), data=panel_df)
mod_nofe_5lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + 
                       lag(Total_layoff, 1) + lag(Total_layoff, 2) + 
                       lag(Total_layoff, 3) + lag(Total_layoff, 4) +
                       lag(Total_layoff, 5), data=panel_df)
...
stargazer(mod_nofe_nolag, mod_nofe_1lag, mod_nofe_2lag, mod_nofe_3lag, mod_nofe_4lag, mod_nofe_5lag,
          align=TRUE, no.space=TRUE, #omit.stat="f",
          dep.var.labels = c("Total deaths from alcohol and drugs in only county-years with data for both"),
          covariate.labels = c("Total layoffs this year",
                               "Total layoffs 1 year ago",
                               "Total layoffs 2 years ago",
                               "Total layoffs 3 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-nofe")
...
## total deaths on total layoffs, no lag. within gives FE model
mod_nolag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff, 
                 model="within", effect = "twoways", data = panel_df)

## total deaths on total layoffs + 1 year lag. FE model
mod_1lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1),
                model="within", effect="twoways", data = panel_df)

## total deaths on total layoff + 1,2 year lag. FE model
mod_2lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1) 
                + lag(Total_layoff, 2), model="within", effect="twoways",
                data=panel_df)

## total deaths on total layoff + 1,2,3 year lag. FE model
mod_3lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3),
                model="within", effect="twoways",data=panel_df)

## total deaths on total layoff + 1,2,3,4 year lag. FE model
mod_4lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                + lag(Total_layoff, 4),
                model="within", effect="twoways", data=panel_df)
## total deaths on total layoff + 1,2,3,4,5 year lag. FE model
mod_5lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                + lag(Total_layoff, 4) + lag(Total_layoff, 5),
                model="within", effect="twoways", data=panel_df)
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with year and county fixed effects",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
...
##### REGRESSIONS ON TOTAL LAYOFFS WITH YEAR-STATE FE #####
## total deaths on total layoffs, no lag. within gives FE model
mod_nolag_ysfe <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + factor(state_years), 
                      model="within", data = panel_df)

## total deaths on total layoffs + 1 year lag. FE model
mod_1lag_ysfe <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + factor(state_years),
                     model="within", effect="twoways", data = panel_df)

## total deaths on total layoff + 1,2 year lag. FE model
mod_2lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1) 
                     + lag(Total_layoff, 2) + factor(state_years), 
                     model="within", effect="twoways",
                     data=panel_df)

## total deaths on total layoff + 1,2,3 year lag. FE model
mod_3lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3) + factor(state_years),
                     model="within", effect="twoways",data=panel_df)

## total deaths on total layoff + 1,2,3,4 year lag. FE model
mod_4lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                     + lag(Total_layoff, 4) + factor(state_years),
                     model="within", effect="twoways", data=panel_df)
## total deaths on total layoff + 1,2,3,4,5 year lag. FE model
mod_5lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                     + lag(Total_layoff, 4) + lag(Total_layoff, 5) + factor(state_years),
                     model="within", effect="twoways", data=panel_df)
...
stargazer(mod_nolag_ysfe, mod_1lag_ysfe, mod_2lag_ysfe, mod_3lag_ysfe, mod_4lag_ysfe, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with year, county, and state-year fixed effects",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-ysfe")

############## Alcohol & Drug county-yrs w/ both only regressions w/ income ####################
##### OLS REGRESSIONS ON TOTAL LAYOFFS #####
mod_nofe_nolag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc, data=panel_df)
mod_nofe_1lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1),
                     data=panel_df)
mod_nofe_2lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2), data=panel_df)
mod_nofe_3lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3), data=panel_df)
mod_nofe_4lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + 
                       lag(Total_layoff, 1) + lag(Total_layoff, 2) + 
                       lag(Total_layoff, 3) + lag(Total_layoff, 4), data=panel_df)
mod_nofe_5lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + 
                       lag(Total_layoff, 1) + lag(Total_layoff, 2) + 
                       lag(Total_layoff, 3) + lag(Total_layoff, 4) +
                       lag(Total_layoff, 5), data=panel_df)
...
stargazer(mod_nofe_nolag, mod_nofe_1lag, mod_nofe_2lag, mod_nofe_3lag, mod_nofe_4lag, mod_nofe_5lag,
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 2 years ago",
                               "Total layoffs 3 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with Per Capita Income",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-nofe")
...
## total deaths on total layoffs, no lag. within gives FE model
mod_nolag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc, 
                 model="within", effect = "twoways", data = panel_df)

## total deaths on total layoffs + 1 year lag. FE model
mod_1lag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1),
                model="within", effect="twoways", data = panel_df)

## total deaths on total layoff + 1,2 year lag. FE model
mod_2lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1) 
                + lag(Total_layoff, 2), model="within", effect="twoways",
                data=panel_df)

## total deaths on total layoff + 1,2,3 year lag. FE model
mod_3lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3),
                model="within", effect="twoways",data=panel_df)

## total deaths on total layoff + 1,2,3,4 year lag. FE model
mod_4lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                + lag(Total_layoff, 4),
                model="within", effect="twoways", data=panel_df)
## total deaths on total layoff + 1,2,3,4,5 year lag. FE model
mod_5lag <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                + lag(Total_layoff, 4) + lag(Total_layoff, 5),
                model="within", effect="twoways", data=panel_df)
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with Per Capita Income and year and county fixed effects",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
...
##### REGRESSIONS ON TOTAL LAYOFFS WITH YEAR-STATE FE #####
## total deaths on total layoffs, no lag. within gives FE model
mod_nolag_ysfe <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + factor(state_years), 
                      model="within", data = panel_df)

## total deaths on total layoffs + 1 year lag. FE model
mod_1lag_ysfe <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + factor(state_years),
                     model="within", effect="twoways", data = panel_df)

## total deaths on total layoff + 1,2 year lag. FE model
mod_2lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1) 
                     + lag(Total_layoff, 2) + factor(state_years), 
                     model="within", effect="twoways",
                     data=panel_df)

## total deaths on total layoff + 1,2,3 year lag. FE model
mod_3lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3) + factor(state_years),
                     model="within", effect="twoways",data=panel_df)

## total deaths on total layoff + 1,2,3,4 year lag. FE model
mod_4lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                     + lag(Total_layoff, 4) + factor(state_years),
                     model="within", effect="twoways", data=panel_df)
## total deaths on total layoff + 1,2,3,4,5 year lag. FE model
mod_5lag_ysfe <- plm(formula=alc_and_drugs_yrs_w_both_only ~ Total_layoff + income_pc + lag(Total_layoff, 1)
                     + lag(Total_layoff, 2) + lag(Total_layoff, 3)
                     + lag(Total_layoff, 4) + lag(Total_layoff, 5) + factor(state_years),
                     model="within", effect="twoways", data=panel_df)
...
stargazer(mod_nolag_ysfe, mod_1lag_ysfe, mod_2lag_ysfe, mod_3lag_ysfe, mod_4lag_ysfe, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with Per Capita Income and year, county, and state-year fixed effects",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-ysfe")
...
############## Alcohol & Drug county-yrs w/ both only Regressions w/ single factor FEs###############
##### with individual effects only ########
## total deaths on total layoffs, no lag. within gives FE model
mod_nolag <- plm(formula = alc_and_drugs_yrs_w_both_only ~ Total_layoff, 
                 model="within", data = panel_df)
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with county fixed effects only",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with year fixed effects only",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with Per Capita Incomecounty fixed effects only",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
...
stargazer(mod_nolag, mod_1lag, mod_2lag, mod_3lag, mod_4lag, mod_5lag, 
          ...
          covariate.labels = c("Total layoffs this year",
                               ...
                               "Total layoffs 4 years ago",
                               "Total layoffs 5 years ago"),
          title="Total deaths from alcohol and drugs in only county-years with data for both regressed on total layoffs with Per Capita Income and year fixed effects only",
          digits=6,
          column.sep.width = "-4pt",
          label="tb:total-fe")
